...
# change to directory of some git repo.
# fd: search (including hidden files, -H) for directories (-t d) that end in .git (\.git$)
# NB - could also use find probably, fd is just nicer
# fzf: select and pipe to stdout
# xargs dirname: call dirname on output of fzf
# cd: wrap whole thing in cd to change to the directory
gcd() {
